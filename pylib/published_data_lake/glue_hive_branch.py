from pylib.published_data_lake.hive_branch import HiveBranch
...
class GlueHiveBranch(Branch):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._hive_branch = HiveBranch(*args, **kwargs)
    ...
    def put_partition(self, branchable_table, partition, create_new_table_if_missing=True):
        return self._glue_branch.put_partition(branchable_table, partition,
                                               create_new_table_if_missing=create_new_table_if_missing)\
               and self._hive_branch.put_partition(branchable_table, partition,
                                        create_new_table_if_missing=create_new_table_if_missing)

    def fork_branch(self, new_branch_name, dbs=None):
        return self._glue_branch.fork_branch(new_branch_name, dbs=dbs)\
               and self._hive_branch.fork_branch(new_branch_name, dbs=dbs)

    def pull_from_branch(self, reference_branch, dbs):
        return self._glue_branch.pull_from_branch(reference_branch, dbs)\
               and self._hive_branch.pull_from_branch(reference_branch, dbs)

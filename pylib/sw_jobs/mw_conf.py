    ...
    def countries(self):
        if not self._countries:
            ...
            if hasattr(countries.get(numeric='840'), 'alpha2'):
                ...
                self._countries = dict([(country_code, countries.get(numeric='%s' % country_code.zfill(3)).alpha2) for country_code in self.conf.get('%s/countries' % self.root).split(',')])
            else:
                logger.debug('using pycountry v==18.12.8')
                self._countries = dict([(country_code, countries.get(numeric='%s' % country_code.zfill(3)).alpha_2) for country_code in self.conf.get('%s/countries' % self.root).split(',')])

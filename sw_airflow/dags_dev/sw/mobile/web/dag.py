...
def assemble_process(mode, dag, sum_ww_value_size):
    ...
    popular_pages_top_store = \
        factory.build(task_id='popular_pages_top_store',
    ...
    adjust_calc = \
        new_algo_factory.build(task_id='adjust_calc',
                               core_command='adjust_est.sh -p redist')
    adjust_calc.set_upstream([gaps_filler, adjust_calc_intermediate])
    # ###########################################################################################################

    check_daily_estimations = \
        factory.build(task_id='check_daily_estimations',
                      core_command='check_daily_estimations.sh')
    check_daily_estimations.set_upstream(adjust_calc)

    calc_subdomains = \
        factory.build(task_id='calc_subdomains',
                      core_command='calc_subdomains.sh')
    calc_subdomains.set_upstream([adjust_calc, prepare_hbase_tables])
    ...
    for i in range(0, sum_ww_value_size):
        ...
        sum_ww_i.set_upstream(adjust_calc)
        sum_ww_all.append(sum_ww_i)

    # load to Hbase
    adjust_store = \
        factory.build(task_id='adjust_store',
                      core_command='adjust_est.sh -p store -ww')
    adjust_store.set_upstream([prepare_hbase_tables] + sum_ww_all)

    # FINISH WINDOW PART
    mobile_web = DummyOperator(task_id=dag.dag_id, dag=dag, sla=timedelta(hours=1))
    ...
    if mode == SNAPHOT_MODE:
        ...
        predict_validate.set_upstream([predict_validate_preparation, adjust_calc])

        compare_est_to_qc = \
            factory.build(task_id='compare_est_to_qc', core_command='compare_estimations_to_qc.sh -sm')

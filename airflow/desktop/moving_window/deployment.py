...
from operators import DockerBashOperator, CopyHbaseTableOperator
from moving_window.window_calculation import *

# Copy tables to production
copy_to_prod = DummyOperator(task_id='copy_to_prod', dag=dag)
...
dynamic_settings_prod = DummyOperator(task_id='dynamic_settings_prod', dag=dag)
...
# Cleanup
cleanup_all = DummyOperator(task_id="cleanup_all", dag=dag)
cleanup_stage_all = DummyOperator(task_id="cleanup_stage_all", dag=dag)
cleanup_prod_all = DummyOperator(task_id="cleanup_prod_all", dag=dag)
...
for offset_days in reversed(range(7, 13)):
    ...
    cleanup_prod = DummyOperator(task_id='cleanup_prod_%d' % offset_days, dag=dag)
...
repair_hive_tables = DummyOperator(task_id='repair_hive_tables', dag=dag)
